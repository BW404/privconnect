...
function createPost($text, $image) {
    ...
    global $conn;
    $query = "INSERT INTO posts (user_id, post_text, post_img) VALUES ('".$_SESSION['userdata']['id']."', '".$text."', '".$post_image."')";
    $run = mysqli_query($conn, $query);
    return $run;
}
...
function getPosts() {
    global $conn;
    $query = "SELECT posts.id, posts.user_id,posts.post_text, posts.post_img, posts.created_at, users.first_name, users.last_name, users.profile_pic FROM posts JOIN users ON posts.user_id = users.id ORDER BY posts.created_at DESC";
    $run = mysqli_query($conn, $query);
