...
function getPosts() {
    global $conn;
    $query = "SELECT posts.id, posts.user_id,posts.post_text, posts.post_img, posts.created_at, users.first_name, users.last_name, users.profile_pic FROM posts JOIN users ON posts.user_id = users.id";
    $run = mysqli_query($conn, $query);
